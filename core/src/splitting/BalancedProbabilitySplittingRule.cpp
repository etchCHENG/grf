...
#include "BalancedProbabilitySplittingRule.h"
...
BalancedProbabilitySplittingRule::BalancedProbabilitySplittingRule(size_t max_num_unique_values,
                                                   size_t num_classes,
                                                   double alpha,
...
bool BalancedProbabilitySplittingRule::find_best_split(const Data& data,
                                               size_t node,
  ...
  arma::mat target_weight_left_sum(num_target_weight_cols, size_node); // column major
  ...
  for (size_t var : possible_split_vars) {
    find_best_split_value(data, node, var, num_classes, class_counts, size_node, min_child_size,
                          ...
                          target_weight_sum, target_weight_left_sum, target_weight_penalty_metric, target_weight_penalty_rate);
  }
...
void BalancedProbabilitySplittingRule::find_best_split_value(const Data& data,
  ...
  for (bool send_left : {true, false}) {
    ...
    for (size_t i = 0; i < num_splits; ++i) {
      ...
      double decrease_left = sum_left / (double)n_left;
      double decrease_right = sum_right / (double)n_right;
